...
#library(httr)
library(jsonlite)
# all the stuff with rnoaa won't knit cause I only got it working on my laptop and now it doesn't even work there anymore, that package is fucked. just trust and believe it worked when the package was usable
...
This data seems to satisfy the requirements for tidy data already
```{r}
...
ocupational_dist_alumn <- ocupational_dist_alumn %>% 
  mutate(
    job_type = X1, 
    percent = X2
         )
...
ggplot(data = ocupational_dist_alumn,
