...
```{r}
#| label: take-a-guess (uh.. like $150 ??)

# add code here

#Getting the point estimate (observed statistic)
abb_x_bar <- abb %>% 
  specify(response = ppg) %>% 
  calculate(stat = "mean")

abb_x_bar
#the real point estimate is ~$76.59
```
...
abb_se_ci <- abb_boot_dis %>% 
  get_confidence_interval(type = "se", point_estimate = abb_x_bar)
visualize(abb_boot_dis) + 
  shade_confidence_interval(endpoints = abb_se_ci)
...
Well, I am 95% confident that in any given sample my CI would capture the true average cost per guest of air bnbs in portanld. In this example my bounds are: ~$62.75 & ~$90.42. which contain my original estimate
